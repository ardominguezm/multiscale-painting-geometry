...
def load_inputs(results_dir: Path):
    ...
    contrasts = pd.read_csv(results_dir / 'classification_contrasts.csv')
    organisation = pd.read_csv(results_dir / 'geometric_organisation.csv')
    return metrics, contrasts, organisation
...
def figure2(res: pd.DataFrame, delt: pd.DataFrame, outdir: Path):
    ...
    save(fig, outdir, 'figure2_complementarity')
    plt.close(fig)
...
def figure3(res: pd.DataFrame, outdir: Path):
    ...
    fig.suptitle('Multiscale discrimination across spatial scales', x=0.02, y=1.04,
                 ha='left', fontsize=16, fontweight='bold')
    save(fig, outdir, 'figure3_scale_discrimination')
    plt.close(fig)
...
def figure4(p5: pd.DataFrame, outdir: Path):
    ...
    fig.suptitle('Style organisation in multiscale geometric space', x=0.02, y=0.975,
                 ha='left', fontsize=16, fontweight='bold')
    save(fig,outdir,'figure4_style_organisation')
    plt.close(fig)


def tables(delt: pd.DataFrame, outdir: Path):
    outdir.mkdir(parents=True, exist_ok=True)
    t1 = pd.DataFrame([
        ['ArtBench-10', 10, 59962, 2126, 5, 'Main analysis'],
        ['WikiArt-8 control', 8, 48000, 1701, 5, 'Source sensitivity'],
    ], columns=['Dataset','Styles','Images','Artists','Folds','Role'])
    t1.to_csv(outdir/'table1_dataset_summary.csv',index=False)
    (outdir/'table1_dataset_summary.tex').write_text(t1.to_latex(index=False,escape=False),encoding='utf-8')
    ...
    t2.to_csv(outdir/'table2_directional_contrasts.csv',index=False)
    (outdir/'table2_directional_contrasts.tex').write_text(t2.to_latex(index=False,escape=False),encoding='utf-8')


def main(results_dir: Path, output_root: Path):
    setup_style()
    res,delt,p5=load_inputs(results_dir)
    figdir=output_root/'figures'; tabdir=output_root/'tables'
    figure2(res,delt,figdir); figure3(res,figdir); figure4(p5,figdir); tables(delt,tabdir)
    print('Paper-output build complete.')
    print('Output:', output_root)


if __name__=='__main__':
    repository = Path(__file__).resolve().parents[1]
    ap=argparse.ArgumentParser(description='Rebuild Figures 2--4 and Tables 1--2 from compact result files.')
    ap.add_argument('--results-dir',type=Path,default=repository/'results')
    ap.add_argument('--output-root',type=Path,default=repository/'build')
    a=ap.parse_args(); main(a.results_dir,a.output_root)
