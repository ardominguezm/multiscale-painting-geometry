...
def pipe():
    # n_samples >> n_features in every representation, so the primal
...
def run(df, label, out, outer=5, nboot=5000):
    ...
    for i, (a, b, h) in enumerate(H):
        ...
        pd.DataFrame(dr).to_csv(out / 'directional_contrasts.partial.csv', index=False)

    D = pd.DataFrame(dr)
    D['q_bh'] = bh(D['p_one_sided'].to_numpy())
    D.to_csv(out / 'directional_contrasts.csv', index=False)
...
def main(inp, out, outer, nboot):
    print('Reading the full feature matrix...', flush=True)
    df = pd.read_csv(inp)
    ...
    for d, label in datasets:
        ...
        allr.append(r)
        alld.append(z)
        pd.concat(allr, ignore_index=True).to_csv(out / 'classification_metrics.partial.csv', index=False)
        pd.concat(alld, ignore_index=True).to_csv(out / 'classification_contrasts.partial.csv', index=False)

    pd.concat(allr, ignore_index=True).to_csv(out / 'classification_metrics.csv', index=False)
    pd.concat(alld, ignore_index=True).to_csv(out / 'classification_contrasts.csv', index=False)
    ...
    print('\nArtist-disjoint evaluation complete.', flush=True)


if __name__ == '__main__':
    p = argparse.ArgumentParser(
        description='Run the fixed artist-disjoint linear evaluation reported in the paper.'
    )
