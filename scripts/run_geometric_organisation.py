...
def main(features_path: Path, output_dir: Path, n_perm: int, seed: int):
    ...
    if "artist" not in df:
        raise KeyError("The feature matrix must contain the artist column.")
    ...
    pd.DataFrame(metadata).to_csv(output_dir / "dataset_summary.csv", index=False)
    summaries = []
    ...
    pd.concat(summaries, ignore_index=True).to_csv(output_dir / "geometric_organisation.csv", index=False)
    print("\nGeometric-organisation analysis complete ->", output_dir)


if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Analyse multiscale geometric organisation by style.")
    p.add_argument("--features", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, default=Path("results/geometric_organisation"))
