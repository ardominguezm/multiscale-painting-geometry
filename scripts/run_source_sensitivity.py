...
from run_geometric_organisation import (
    SCALES,
...
def main(features: Path, output_dir: Path, n_perm: int, seed: int):
    ...
    primary.to_csv(output_dir / "source_composition_sensitivity.csv", index=False)
    ...
    loo.to_csv(output_dir / "leave_one_style_out_sigma2.csv", index=False)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Evaluate source-composition sensitivity of the artist-centroid style effect."
    )
    p.add_argument("--features", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, default=Path("results/source_sensitivity"))
